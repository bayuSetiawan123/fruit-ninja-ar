...
def check_interaction(rect, nose_pos):
    x, y, w, h = rect
    hit_nose = False
    if nose_pos:
        if (x < nose_pos[0] < x+w) and (y < nose_pos[1] < y+h):
            hit_nose = True
    hit_mouse = False
    if (x < mouse_x < x+w) and (y < mouse_y < y+h) and mouse_clicked:
        hit_mouse = True
    return hit_nose or hit_mouse
...
def draw_button(frame, text, x,y,w,h, color=(50,200,50), text_col=(255,255,255), active=True):
    final_color = color if active else (100,100,100)
    cv2.rectangle(frame, (x,y),(x+w,y+h), final_color, -1)
    cv2.rectangle(frame, (x,y),(x+w,y+h), (255,255,255), 2)
    tsize = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
    tx = x + (w - tsize[0]) // 2; ty = y + (h + tsize[1]) // 2
    cv2.putText(frame, text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.8, text_col, 2)
...
# MAIN LOOP
# ================================
# MENAMBAHKAN STATE_PRE_RECAP (ID: 6) UNTUK TRANSISI
...
# [BARU] Variable untuk menyimpan list foto (Recap)
photo_gallery = [] 
current_slide_idx = 0
...
try:
    while True:
        ...
        if game_state == STATE_MENU:
            ...
            cv2.putText(frame, f"High Score: {high_score}", (220, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,215,0), 2)
            
            start_color = (0,200,0) if camera_active else (50,50,50)
            draw_button(frame, "START", 220, 200, 200, 60, color=start_color, active=(input_allowed and camera_active))
            draw_button(frame, "EXIT", 220, 280, 200, 60, color=(0,0,200), active=input_allowed)

            if not camera_active:
                btn_cam_rect = (200, 360, 240, 50)
                draw_button(frame, "NYALAKAN KAMERA", *btn_cam_rect, color=(0, 200, 200), active=input_allowed)
                if input_allowed and check_interaction(btn_cam_rect, nose):
                    video_stream.start(); camera_active = True; state_timer = 20 
            else:
                btn_cam_rect = (220, 360, 200, 50)
                draw_button(frame, "MATIKAN KAMERA", *btn_cam_rect, color=(150, 50, 50), active=input_allowed)
                if input_allowed and check_interaction(btn_cam_rect, nose):
                    video_stream.stop(); camera_active = False; state_timer = 20

            if input_allowed and camera_active:
                if check_interaction((220, 200, 200, 60), nose): 
                    game_state = STATE_LEVEL_SELECT; state_timer = 30 
            if input_allowed:
                if check_interaction((220, 280, 200, 60), nose): break
                draw_button(frame, "RESET", 520, 400, 100, 50, color=(200,50,50), active=input_allowed)
                if check_interaction((520, 400, 100, 50), nose):
                    high_score = 0; save_high_score(0)
                    cv2.putText(frame, "RESET!", (530, 390), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)

        # 2. PILIH LEVEL
        elif game_state == STATE_LEVEL_SELECT:
            cv2.putText(frame, "PILIH LEVEL (1-5)", (200, 80), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255,255,0), 2)
            btn_w, btn_h = 70, 70; gap = 20
            total_w = (5 * btn_w) + (4 * gap); start_x = (FRAME_W - total_w) // 2; btn_y = 350
            for lvl in range(1, 6):
                bx = start_x + (lvl-1)*(btn_w+gap)
                col = (50, 255 - (lvl*40), 50 + (lvl*40))
                draw_button(frame, str(lvl), bx, btn_y, btn_w, btn_h, color=col, active=input_allowed)
                if input_allowed and check_interaction((bx, btn_y, btn_w, btn_h), nose):
        ...
        # 4. GAMEPLAY
            ...
            if timer_frames <= 0: 
                ...
                # [TRANSISI] Masuk ke PRE_RECAP dulu, bukan langsung SNAPSHOT
                game_state = STATE_PRE_RECAP 
                state_timer = 180 # Jeda 2 detik sebelum recap dimulai
                
                if score > high_score: high_score = score; save_high_score(high_score)

        # 5. [BARU] STATE PRE_RECAP (Jeda sebelum Slideshow)
        elif game_state == STATE_PRE_RECAP:
            # Tampilkan frame terakhir dengan sedikit gelap biar teks terbaca
            ...
            if state_timer <= 0:
                game_state = STATE_SNAPSHOT # Pindah ke Slideshow
                state_timer = 200 # Reset timer untuk slideshow (5 detik)
                current_slide_idx = -1

        # 6. STATE SNAPSHOT (Slideshow Recap 3 Foto)
        elif game_state == STATE_SNAPSHOT:
            # Hitung slide
            slide_duration = 50
            new_slide_idx = 2 - (state_timer // slide_duration) # 0, 1, 2
            new_slide_idx = max(0, min(2, new_slide_idx)) 

            # Play Sound hanya saat slide BERUBAH
            if new_slide_idx != current_slide_idx:
                current_slide_idx = new_slide_idx
                if shutter_sound: shutter_sound.play() # CEKREK!
            ...
            # Judul Recap
            cv2.rectangle(frame, (0, 0), (FRAME_W, 60), (0,0,0), -1) # Background Hitam atas
            ...
            # Jika timer habis, pindah ke menu hitam
            if state_timer <= 0:
                game_state = STATE_GAMEOVER

        # 7. GAMEOVER (Menu Standar - Background Hitam)
        elif game_state == STATE_GAMEOVER:
            frame = blank_frame.copy()

            cv2.putText(frame, "GAME OVER", (170,140), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0,0,255), 4)
            cv2.putText(frame, f"Final Score: {score}", (200,200), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,255,255), 2)
            cv2.putText(frame, f"Best: {high_score}", (240, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,215,0), 2)
            
            btn_menu_rect = (250, 320, 140, 70)
            draw_button(frame, "MENU", *btn_menu_rect, color=(100,200,100), active=input_allowed)
            
            if input_allowed and check_interaction(btn_menu_rect, nose):
